...
export default function ServiceLandingPage() {
  ...
  return (
    <div className="min-h-screen theme-bg theme-text-primary transition-colors duration-300">
      <SEO
        title={page.title}
        description={page.description}
      ...
      <section className="relative overflow-hidden border-b theme-border pt-32 pb-16 md:pt-44 md:pb-28">
        <div className="absolute inset-0 premium-grid-overlay opacity-20 pointer-events-none" />
        <div className="absolute right-0 top-24 h-96 w-96 rounded-full bg-brand-cyan/10 blur-[130px] pointer-events-none" />
        <div className="absolute bottom-0 left-0 h-96 w-96 rounded-full bg-brand-blue/10 blur-[130px] pointer-events-none" />
        <div className="container relative z-10 mx-auto max-w-7xl px-5 sm:px-6 md:px-8">
          <Reveal className="max-w-4xl">
            ...
            <h1 className="text-4xl font-display font-medium uppercase leading-[1.05] tracking-tight md:text-7xl theme-text-primary">
              {page.h1}
            </h1>
            <p className="mt-6 max-w-3xl text-base font-light leading-relaxed theme-text-muted md:text-xl">
            ...
            <div className="mt-9 flex flex-col gap-3 sm:flex-row">
              ...
              <Link to={p('/case-studies')}>
                <Button variant="outline" className="h-13 w-full border theme-border theme-bg-tertiary px-7 text-[10px] font-mono font-bold uppercase tracking-[0.18em] sm:w-auto">
                  View Proof
                </Button>
              </Link>
            </div>
          ...
          <StaggerContainer className="mt-12 grid grid-cols-1 gap-3 sm:grid-cols-3 md:mt-16">
            {[
              ...
              return (
                <StaggerItem key={item.label} className="rounded-xl border theme-border theme-card p-5">
                  ...
                  <p className="text-xs font-light leading-relaxed theme-text-muted">{item.value}</p>
                </StaggerItem>
              );
            })}
          </StaggerContainer>
        ...
        <div className="container mx-auto grid max-w-7xl grid-cols-1 gap-12 px-4 md:px-8 lg:grid-cols-[0.72fr_0.28fr]">
          <div className="space-y-14">
            {page.sections.map((section, index) => (
              <Reveal key={section.title} className="border-b theme-border pb-12 last:border-b-0 last:pb-0">
                ...
                <h2 className="mb-5 text-2xl font-display font-medium uppercase leading-tight tracking-tight theme-text-primary md:text-4xl">
                  {section.title}
                </h2>
                <div className="space-y-5">
                  {section.paragraphs.map((paragraph) => (
                    <p key={paragraph} className="text-sm font-light leading-8 theme-text-muted md:text-base">
                      {paragraph}
                    </p>
                  ))}
                </div>
          ...
          <aside className="lg:sticky lg:top-28 lg:self-start">
            <div className="rounded-2xl border theme-border theme-card p-6 shadow-md">
              <p className="mb-4 text-[10px] font-mono font-bold uppercase tracking-[0.24em] text-brand-cyan">
                Internal Links
              </p>
              <div className="grid gap-2">
                <Link to={p('/services')} className="rounded-lg border theme-border px-4 py-3 text-xs theme-text-muted transition hover:border-brand-cyan/40 hover:theme-text-primary">
                  All Services
                </Link>
                <Link to={p('/pricing')} className="rounded-lg border theme-border px-4 py-3 text-xs theme-text-muted transition hover:border-brand-cyan/40 hover:theme-text-primary">
                  Pricing
                </Link>
                <Link to={p('/process')} className="rounded-lg border theme-border px-4 py-3 text-xs theme-text-muted transition hover:border-brand-cyan/40 hover:theme-text-primary">
                  Delivery Process
                </Link>
                <Link to={p('/case-studies')} className="rounded-lg border theme-border px-4 py-3 text-xs theme-text-muted transition hover:border-brand-cyan/40 hover:theme-text-primary">
                  Case Studies
                </Link>
                {page.relatedServices.map((service) => (
                  <Link key={service.label} to={p(service.path)} className="rounded-lg border theme-border px-4 py-3 text-xs theme-text-muted transition hover:border-brand-cyan/40 hover:theme-text-primary">
                    {service.label}
                  </Link>
                ))}
              </div>
        ...
        <div className="container mx-auto max-w-7xl px-4 md:px-8">
          ...
          <StaggerContainer className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
            {page.deliverables.map((item) => (
              <StaggerItem key={item} className="rounded-xl border theme-border theme-card p-5">
                <CheckCircle2 className="mb-4 h-5 w-5 text-brand-cyan" />
                <p className="text-sm font-light leading-relaxed theme-text-secondary">{item}</p>
              </StaggerItem>
            ))}
          </StaggerContainer>
        </div>
        ...
        <div className="container mx-auto max-w-7xl px-4 md:px-8">
          ...
          <StaggerContainer className="grid grid-cols-1 gap-4 md:grid-cols-3">
            {[
              ...
              const Icon = item.icon;
              return (
                <StaggerItem key={item.title} className="rounded-xl border theme-border theme-card p-6">
                  <Icon className="mb-5 h-5 w-5 text-brand-cyan" />
                  <h3 className="mb-3 text-lg font-display font-medium uppercase tracking-tight theme-text-primary">{item.title}</h3>
                  <p className="text-sm font-light leading-relaxed theme-text-muted">{item.copy}</p>
                </StaggerItem>
              );
            })}
          </StaggerContainer>
      ...
      <section className="relative border-y theme-border theme-bg-tertiary py-20 md:py-28">
        <div className="container mx-auto grid max-w-7xl grid-cols-1 gap-6 px-4 md:px-8 lg:grid-cols-2">
          <Reveal className="rounded-xl border theme-border theme-card p-6 md:p-8">
            <span className="mb-4 block text-[10px] font-mono font-bold uppercase tracking-[0.26em] text-brand-cyan">
              Technologies
            </span>
            <h2 className="mb-6 text-2xl font-display font-medium uppercase leading-tight tracking-tight theme-text-primary md:text-4xl">
              Modern stack, search-ready structure
            </h2>
            <div className="grid gap-3 sm:grid-cols-2">
              {page.technologyAngles.map((technology) => (
                <div key={technology} className="flex items-start gap-3 rounded-lg border theme-border px-4 py-3 text-xs font-light leading-relaxed theme-text-secondary">
          ...
          <Reveal delay={0.1} className="rounded-xl border theme-border theme-card p-6 md:p-8">
            <span className="mb-4 block text-[10px] font-mono font-bold uppercase tracking-[0.26em] text-brand-cyan">
              Case Studies
            </span>
            <h2 className="mb-5 text-2xl font-display font-medium uppercase leading-tight tracking-tight theme-text-primary md:text-4xl">
            ...
            <p className="mb-8 text-sm font-light leading-7 theme-text-muted md:text-base">
            ...
            <Link to={relatedCasePath}>
              <Button variant="outline" className="h-12 w-full border theme-border theme-bg-tertiary px-7 text-[10px] font-mono font-bold uppercase tracking-[0.18em] sm:w-auto">
                View Relevant Proof
                <ArrowRight className="ml-3 h-4 w-4" />
              </Button>
            </Link>
        ...
        <div className="container mx-auto max-w-7xl px-4 md:px-8">
          ...
          <StaggerContainer className="grid grid-cols-1 gap-4 md:grid-cols-4">
            {page.process.map((item) => (
              <StaggerItem key={item.step} className="rounded-xl border theme-border theme-card p-5">
                <span className="mb-4 block text-sm font-mono font-bold text-brand-cyan">{item.step}</span>
                <h3 className="mb-3 text-lg font-display font-medium uppercase tracking-tight theme-text-primary">{item.title}</h3>
                <p className="text-xs font-light leading-relaxed theme-text-muted">{item.copy}</p>
              </StaggerItem>
            ))}
          </StaggerContainer>
        </div>
      ...
      <section className="relative overflow-hidden py-20 md:py-28">
        <div className="absolute inset-x-0 bottom-0 h-72 bg-gradient-to-t from-brand-blue/12 to-transparent pointer-events-none" />
        <div className="container relative z-10 mx-auto max-w-4xl px-4 text-center md:px-8">
          <Reveal>
            <h2 className="text-3xl font-display font-medium uppercase leading-tight tracking-tight theme-text-primary md:text-6xl">
              Ready to make this service visible?
            </h2>
            <p className="mx-auto mt-5 max-w-2xl text-sm font-light leading-relaxed theme-text-muted md:text-lg">
            ...
            <div className="mt-9 flex flex-col justify-center gap-3 sm:flex-row">
              ...
              <a href={config.whatsappLink} target="_blank" rel="noreferrer">
                <Button variant="outline" className="h-13 w-full border theme-border theme-bg-tertiary px-8 text-[10px] font-mono font-bold uppercase tracking-[0.18em] sm:w-auto">
                  WhatsApp
                </Button>
              </a>
            </div>
